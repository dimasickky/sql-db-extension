"""sql-db · SqlDbExtension instance, the backend HTTP client, Fernet helpers.
...
def encrypt_password(plaintext: str) -> str:
    ...
    the backend carries ``password_encrypted`` only, and the backend / MariaDB
...
def build_conn_info(conn: dict) -> dict:
    """Serialise a saved connection row into the wire shape the backend wants."""
    return {
        "host":               conn["host"],
