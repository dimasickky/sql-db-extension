...
**BREAKING · SDK v2.0.0 / the platform migration.**
...
preserved; wire contract with the backend (the backend host:8099) unchanged.
...
- **`_system_prompt` + `system_prompt.txt`** (the platform invariant).
...
- Wire contract to the backend (paths, `user_id` in query, `connection`
